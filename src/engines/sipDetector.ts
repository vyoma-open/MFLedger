...
export function detectSipPattern(candidates: InvestmentCandidate[]): Map<string, DetectionResult> {
  ...
  for (const [, symBuys] of bySymbol) {
    ...
    // We track candidate chains: indices in symBuys
...
export interface SipTranche {
  id: string;
  installmentAmountPaise: number; // nominal installment amount (e.g. 200000 paise = ₹2,000)
  startDate: number;
  endDate: number | null; // null if actively ongoing
...
export interface SipStreamAnalytics {
  ...
  // Tranches breakdown: distinct rows for each SIP amount phase (e.g., ₹2000 then ₹3000)
  tranches: SipTranche[];
  // Overall SIP metrics
  sipInvested: number;
  sipUnits: number;
  ...
  latestSipAmountPaise: number; // Actual current/latest tranche installment amount (NOT an average!)
...
/**
 * Determine nominal installment amount (rounds minor stamp duty / paise fluctuation to nearest whole rupee).
 */
...
/**
 * Checks if two installment amounts belong to the same nominal SIP tier.
 */
...
/**
 * Segregates active & historical lots for a holding into SIP stream vs Lump Sum,
 * computing distinct metrics, dates, and XIRR for each.
 * Intelligently recognizes SIP amount changes (e.g. ₹2,000 -> ₹3,000) and produces
 * separate tranche records instead of averaging.
 */
export function analyzeSipStreams(
  ...
  for (const lot of allLots) {
    // 1. Explicit lot tag takes highest priority
    if (lot.investment_type === 'SIP') {
      sipLots.push(lot);
    } else if (lot.investment_type === 'LUMPSUM') {
      lumpLots.push(lot);
    } else {
      // 2. Fall back to pattern detection result
  ...
  // Partition SIP lots into distinct tranches based on nominal installment amount changes and long pauses
  ...
  for (let i = 0; i < sipLots.length; i++) {
    ...
    const longGap = daysDiff > 95; // more than 3 months without installment

    if (sameAmount && !longGap) {
      currentGroup.push(lot);
    } else {
      // Close current group and start new tranche
  ...
  // Build enriched SipTranche objects
  ...
  // Aggregate SIP XIRR calculation across all SIP lots
  const sipXirr = (() => {
...
export interface EnrichedSipStream {
  ...
  name: string;
  symbol: string;
  monthlyAmount: number; // in rupees (actual current/latest tranche amount, NOT an average)
  accumulatedWealth: number; // in rupees
  totalInvested: number; // in rupees
  historicalXirr: number | null; // e.g. 0.165 for 16.5%
